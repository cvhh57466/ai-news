  ...
  for (const feedConfig of FEEDS) {
    try {
      ...
      for (const item of feed.items) {
        ...
        const keyword = title.substring(0, 15);
        const slug = slugify(title.substring(0, 30), { lower: true, remove: /[*+~.()'"!:@]/g }) + '-' + Math.random().toString(36).substring(2, 6);
        const id = Math.random().toString(36).substring(2, 9);
        
        // Extract content (Google News sometimes gives HTML in description)
        let htmlContent = item['content:encoded'] || item.content || item.description || title;
        // Strip basic HTML for pure markdown, but keeping it simple for now, we will just wrap in markdown if it's plain text.
        // Google news descriptions are just links, so let's make it look like a real article
