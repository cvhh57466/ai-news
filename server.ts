...
try {
  ...
  console.warn("Could not load firebase-applet-config.json from require, trying fs...", e);
  try {
     const __dirname = path.dirname(fileURLToPath(import.meta.url));
     const configContent = fs.readFileSync(path.join(__dirname, 'firebase-applet-config.json'), 'utf-8');
     firebaseConfig = JSON.parse(configContent);
  } catch(e2) {
     console.warn("Could not load firebase-applet-config.json from fs either.", e2);
  }
}
...
async function generateRegionalNews(region: string, language: string = 'zh') {
  ...
  try {
    ...
    // If DB has no posts either, return a dummy post instead of an error so the frontend doesn't break
    const dummyPost = {
       id: 'dummy-' + Date.now(),
       title: "今日重點新聞回顧",
       title_en: "Today's Top News",
       slug: 'dummy-news-' + Date.now(),
       content: "目前自動生成新聞額度已達上限，這是系統為您提供的備用新聞。全球供應鏈正經歷新一輪調整，隨著科技發展與地區經濟整合加劇，各行各業都在積極應對市場變化。專家指出，未來幾個季度的產業轉型將更為關鍵。\n\n本報導由系統自動生成，感謝您的諒解。",
       content_en: "Our automatic news generation quota has been reached. This is a fallback news provided by the system. The global supply chain is undergoing a new round of adjustments. With the development of technology and regional economic integration, various industries are actively responding to market changes. Experts point out that industrial transformation in the coming quarters will be even more critical.\n\nThis report is automatically generated by the system. Thank you for your understanding.",
       keyword: "全球趨勢",
       region: region,
       thumbnail: "https://image.pollinations.ai/prompt/global%20news%20minimal%20modern%20style?width=800&height=600&nologo=true",
       category: '熱門',
       createdAt: new Date().toISOString()
    };
    return { post: dummyPost };
  }
}
...
- STRICTLY PROHIBITED: Do not include any phrases like "Written by AI", "As an AI language model", or "Generated by AI".`;
    ...
    const isQuotaError = error.status === 429 || error.message?.includes('429') || error.message?.includes('Quota exceeded') || error.message?.includes('quota');
    if (!isQuotaError) {
    ...
    if (error.message?.includes('API key not valid') || error.message?.includes('API_KEY_INVALID')) {
       ...
       // Fallback post if database has nothing
       const fallbackPost = {
           id: 'dummy-' + Date.now(),
           title: "今日重點新聞回顧：" + keyword,
           title_en: "Today's Top News: " + keyword,
           slug: 'dummy-news-' + Date.now(),
           content: "目前自動生成新聞額度已達上限，這是系統為您提供的備用新聞。關於「" + keyword + "」的最新趨勢，專家指出市場正處於高度變化階段。未來發展值得持續關注。\n\n本報導由系統自動生成，感謝您的諒解。",
           content_en: "Our generation quota is currently reached. Regarding '" + keyword + "', experts suggest the market is highly volatile. We will keep you updated.\n\nGenerated automatically via fallback system.",
           keyword: keyword,
           region: 'Global',
           thumbnail: `https://image.pollinations.ai/prompt/${encodeURIComponent(keyword)}?width=800&height=600&nologo=true`,
           category: '熱門',
           createdAt: new Date().toISOString()
       };
